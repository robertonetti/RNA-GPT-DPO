...
from src_precomputed.dpo_config import Config
from src_precomputed.dpo_logging import print_eval_summary
...
from src_precomputed.dpo_plotting import (
...
def _evaluate_model_state(
    ...
    val_1_enabled: bool,
    val_2_enabled: bool,
    val_1_good_data: torch.Tensor,
    val_1_bad_data: torch.Tensor,
    val_2_good_data: torch.Tensor,
    val_2_bad_data: torch.Tensor,
    dn_eval_data: torch.Tensor,
    val_1_good_labels: torch.Tensor,
    val_1_bad_labels: torch.Tensor,
    val_2_good_labels: torch.Tensor,
    val_2_bad_labels: torch.Tensor,
    ...
    mean_like_ref_precomputed: float,
) -> Dict[str, Any]:
    ...
    nll_train_good_rand = compute_random_batch_nll(
        ...
        batch_size=eval_bs,
        labels=dpo_dataset["good_labels"],
    )
    ...
    if val_enabled:
        nll_val_good_rand = compute_random_batch_nll(
            ...
            n_batches=cfg.nll_rand_n_batches,
            batch_size=eval_bs,
            labels=val_dpo_dataset["good_labels"],
        )
        nll_val_bad_rand = compute_random_batch_nll(
            ...
            n_batches=cfg.nll_rand_n_batches,
            batch_size=eval_bs,
            labels=val_dpo_dataset["bad_labels"],
        )
    else:
    ...
    mean_like_model = compute_mean_token_likelihood(
        model, dn_eval_data, pad_token, device, batch_size=full_eval_bs, labels=dn_eval_labels
    )
    mean_like_ref = mean_like_ref_precomputed
    ...
    if cfg.compute_val_separation_metric:
        _, train_good_seq_nll, train_bad_seq_nll = compute_val_separation_correlation(
            ...
            good_labels=dpo_dataset["good_labels"],
            bad_labels=dpo_dataset["bad_labels"],
        )
        ...
        if val_enabled:
            val_sep_corr, val_good_seq_nll, val_bad_seq_nll = compute_val_separation_correlation(
                ...
                device,
                batch_size=full_eval_bs,
                good_labels=val_dpo_dataset["good_labels"],
                bad_labels=val_dpo_dataset["bad_labels"],
            )
        ...
        if val_1_enabled:
            val_1_sep_corr, val_1_good_seq_nll, val_1_bad_seq_nll = compute_val_separation_correlation(
                ...
                device,
                batch_size=full_eval_bs,
                good_labels=val_1_good_labels,
                bad_labels=val_1_bad_labels,
            )
        ...
        if val_2_enabled:
            val_2_sep_corr, val_2_good_seq_nll, val_2_bad_seq_nll = compute_val_separation_correlation(
                ...
                device,
                batch_size=full_eval_bs,
                good_labels=val_2_good_labels,
                bad_labels=val_2_bad_labels,
            )
