    ...
    while global_iteration < cfg.max_iterations:
        ...
        for batch in train_loader:
            ...
            if cfg.reint:
                ...
                # For the reint loss, we also want to track the combined set of unique sequences to use KD loss on the full set of unique sequences in the batch (not just the preferred ones).
                x_l_unique = torch.unique(x_l, dim=0)
                y_l_unique = _build_labels_from_inputs(x_l_unique, pad_token)
                x_kl = torch.cat([x_w, x_l_unique], dim=0)
                y_kl = torch.cat([y_w, y_l_unique], dim=0)

                kl_loss = compute_batched_token_level_kd_loss_from_tensors(
                    model,
                    ref_model,
                    x_kl,
                    y_kl,
                ...
                loss = loss + cfg.lambda_kl * kl_loss
